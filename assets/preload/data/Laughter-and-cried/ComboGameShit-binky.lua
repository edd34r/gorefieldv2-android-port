-- Settings (Player) --
    ...
    local show = { -- Set an item to false to hide it from view
        ...
        missSprite = false, -- "Rating" that shows every note miss (and ghost miss)
    ...
    local path = {
        ratings = 'binky/', -- starts in images/
        nums = 'binky/'
    }
    ...
    local ratingPos = {
        game   = {x = 1390, y = 550},  -- camGame position
...
function onCreatePost()
    ...
    -- PLAYER nil checks!!!! --
    ratingPos.game.x = (ratingPos.game.x or dadDefault.x)
    ratingPos.game.y = (ratingPos.game.y or dadDefault.y)
    ...
    -- DAD nil checks!!!! --
    ratingPosOp.game.x = (ratingPosOp.game.x or bdDefault.x)
    ratingPosOp.game.y = (ratingPosOp.game.y or bdDefault.y)
    ...
    if not inGameOver then 
        ...
        if constantGameCam then
            if camSet == 'game' and visible then -- no point in doing it if not on game cam
                bf1 = getMidpointX('dad') - (getProperty('boyfriend.width') / 2) - 120
                bf2 = ((getMidpointY('dad') - (getProperty('boyfriend.height') / 1.7)) / (pixel and 1.5 or 1))
            ...
            if camSetOp == 'game' and visibleOp then
                dad1 = getMidpointX('bf') + (getProperty('dad.width') / (2 * (pixelOp and -5 or 1))) -- pico is a motherfucker
                dad2 = getMidpointY('bf') - (getProperty('dad.height') / (pixelOp and 2 or 4))
